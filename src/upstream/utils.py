...
def save_expert_traj(
    ...
    model,
    spec_kwargs,
    extra_reward_threshold=0,
    nr_trajectories=10,
    stable_baselines_model=False,
    pl_model_file=None,
):
    num_steps = 0
    expert_traj = []
    ...
    for i_episode in count():
        ...
        while not done:
            if stable_baselines_model:
                ...
                ac = model.get_action(
                    torch.from_numpy(ob).type(torch.get_default_dtype())
                )
    ...
    if not os.path.exists(opt.demo_dir):
        ...
        os.mkdir(
            os.path.join(
                opt.demo_dir, "preference_learning", opt.env_name + spec_kwargs
            )
        )
    ...
    if pl_model_file is not None:
        np.save(
            os.path.join(
                opt.demo_dir, "preference_learning/" + filename + "_expert_traj.npy"
            ),
            expert_traj,
        )
    else:
    ...
    if len(expert_traj_extra) > 0 and pl_model_file is not None:
        ...
        np.save(
            os.path.join(opt.demo_dir, filename + "_expert_traj_extra.npy"),
            expert_traj_extra,
        )
...
def save_ranked_expert_demos(opt, model_dir, env_spec):
    ...
    for model_file in os.listdir(model_dir):
        ...
        save_expert_traj(
            ...
            model,
            env_spec,
            stable_baselines_model=True,
            pl_model_file=model_file,
        )
        ...
        if env_spec is None:
            ...
            for i, spec in enumerate(env_spec):
                demos = np.load(
                    os.path.join(
                        opt.demo_dir,
                        env_name + format_name_string(str(spec)) + extra_string,
                    )
                )
...
def prepare_update_gail(env, opt, expert_demos, obs, acs):
    ...
    expert_ob_ac_done_reward = expert_demos[
        np.random.randint(0, expert_demos.shape[0], opt.n_steps * opt.n_envs), :
    ]
...
def process_sb_args(sb_args):
    ...
    sb_args = without_keys(
        sb_args, "n_envs", "n_timesteps", "policy", "env_wrapper", "normalize"
    )
...
def train_using_sb(
    opt, save_checkpoints_for_pl=None, pref_reward_model=None, irl_reward_model=None
):
    """
    ...
    if irl_reward_model is not None:
        ...
        opt_test = json.load(
            open(os.path.join(os.path.dirname(irl_reward_model), "args.json"))
        )
        opt_test = Namespace(**opt_test)
        if opt.discriminator_type == "airl":
            from irl import AIRLDiscriminator

            disc_test = AIRLDiscriminator(
                gym.make(opt_test.env_name, **opt.env_spec_test),
    ...
    for spec_kwargs in list_kwargs:
        ...
        env, test_env = make_venv(
            opt, opt.n_envs, spec_kwargs, opt.env_spec_test, wrapper_kwargs
        )
        sb_args = process_sb_args(sb_args)
        ...
        model_filename = (
            "sb_models/ppo2_" + env_name + format_name_string(str(spec_kwargs))
        )
        ...
        if not opt.load_sb_model:
            if save_checkpoints_for_pl is not None:
                checkpoint_callback = CheckpointCallback(
                    save_freq=10000,
                    save_path=save_checkpoints_for_pl,
                ...
                opt.pretrained_pref_reward is not None
                or opt.pretrained_irl_reward is not None
            ):
        ...
        if save_checkpoints_for_pl is not None:
            ...
            if "MiniGrid" in env_name:
                ...
                _, eval_env = make_venv(
                    opt,
                    n_envs=1,
                    spec=spec_kwargs,
                    spec_test=spec_kwargs,
                    wrapper_kwargs={},
                )
            ...
            save_expert_traj(
                ...
                nr_trajectories=opt.num_expert_traj,
                stable_baselines_model=True,
            )
...
class CustomEpRewardCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        self.aux_writer.add_scalar(
            "Reward/Ep_rewards_gt", self.episode_reward_gt, self.cnt
        )
