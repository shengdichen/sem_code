...
from irl import AIRLDiscriminator, SWILDiscriminator, GAILDiscriminator,\
                VAILDiscriminator, MEIRLDiscriminator, WAILDiscriminator
...
class MovePoint(Env):
    ...
    def draw_elements_on_canvas(self):
        ...
        self.canvas_hist[agent.y : agent.y + agent_shape[1], agent.x:agent.x + agent_shape[0]] += 1
    ...
    def reset(self):
        ...
        if self.random_init:
            x = random.randrange(int(self.canvas_shape[0] * 0.05), int(self.canvas_shape[0] * 0.10))
            y = random.randrange(int(self.canvas_shape[1] * 0.15), int(self.canvas_shape[1] * 0.20))
        else:
            x = 10
            y = 10
        ...
        pos = [(int(self.canvas_shape[0]/2)+self.shift_x,int(self.canvas_shape[1]/2)+self.shift_y),
               (int(self.canvas_shape[0]*.95),int(self.canvas_shape[1]*.95))]
        self.targets = []
        for i,p in enumerate(pos):
            tgt = Target("tgt_{}".format(i), self.x_max, self.x_min, self.y_max, self.y_min)
    ...
    def generate_random_targets(self):
        ...
        for i in range(self.n_tgt):
            ...
            tgt_x = random.randrange(self.y_min+int(self.y_max/4), self.y_max-int(self.y_max/4))
            tgt_y = random.randrange(self.y_min+int(self.y_max/4), self.y_max-int(self.y_max/4))
    ...
    def step(self, action):
        ...
        reward = -(np.sqrt((self.agent.x - curr_tgt.x)**2 + (self.agent.y-curr_tgt.y)**2))
...
class RewardCheckpointCallback(BaseCallback):
    def __init__(self, discriminator, verbose=0, id="", plot_value=False,
                 log_path=None):
    ...
    def _on_step(self) -> bool:
        ...
        if self.plot_value:
            self.plot_list_value.append(plot_reward(self.discriminator, plot_value=True))
        torch.save(self.discriminator.state_dict(), os.path.join(self.log_path, 'best_disc.th'))
        return True
...
def save_expert_traj(env, model, nr_trajectories=10,
                     render=False, demo_dir='./demos', filename='exp',
                     deterministic=False):
    num_steps = 0
    expert_traj = []
...
def load_expert_demos(n_timesteps=3e5):
    expert_demos = []
    expert_demos.append(np.load('demos/exp_0_0' + str(n_timesteps) + '_expert_traj.npy'))
    expert_demos.append(np.load('demos/exp_50_0' + str(n_timesteps) + '_expert_traj.npy'))
    expert_demos.append(np.load('demos/exp_0_50' + str(n_timesteps) + '_expert_traj.npy'))
...
def plot_reward(discriminator, plot_value=False, env=None):
    ...
    grid_pos1 = np.concatenate([point_list, np.tile(tgt_pos_1, (point_list.shape[0],1))],1)
    grid_pos2 = np.concatenate([point_list, np.tile(tgt_pos_2, (point_list.shape[0],1))],1)

    def dist_f(grid_pos):
         return -(np.sqrt((grid_pos[:,0] - grid_pos[:,2])**2 + (grid_pos[:,1]-grid_pos[:,3])**2))
...
def train_expert(n_timesteps, n_targets, shift_x, shift_y, kwargs,
                 fname, model_dir='./models', save_deterministic=False):
    ...
    save_expert_traj(env, model, nr_trajectories=10, render=False,
                     filename=fname+str(n_timesteps), deterministic=save_deterministic)

    return model


def train_pwil(demos, n_demos, subsampling, use_actions,
               n_timesteps, n_targets, shift_x, shift_y, kwargs,
               fname, model_dir='./models', save_deterministic=False):
    env = PWILReward(env=MovePoint(n_targets, shift_x, shift_y),
                     demos=demos,
                     n_demos=n_demos,
    ...
    eval_callback = EvalCallback(testing_env, best_model_save_path=log_path,
                                                    log_path=log_path, eval_freq=10000,
                                                    deterministic=True, render=False)

    #eval_callback.init_callback(ppo_dict[k])
    callback_list = CallbackList([CustomCallback(id='', log_path=log_path), eval_callback, TqdmCallback()])
...
def train_irl(opt, opt_policy, seed):
    ...
    log_path = './logs/results_2dnav_irl' + datetime.now().strftime('%Y%m%d_%H%M%S') + log_suffix
    ...
    if opt.discriminator_type == 'airl':
        discriminator = AIRLDiscriminator(env, opt.d_layer_dims,
                                        lr=opt.lr,
                                        gamma=opt.gamma,
        ...
        discriminator = SWILDiscriminator(env, opt.d_layer_dims,
                                                  lr=opt.lr,
                                                  batch_size=opt.batch_size,
                                                  use_actions=opt.use_actions,
                                                  n_proj=opt.n_proj)
    elif opt.discriminator_type == 'vail':
        discriminator = VAILDiscriminator(env, opt.d_layer_dims,
                                        lr=opt.lr,
        ...
        discriminator = GAILDiscriminator(env, opt.d_layer_dims,
                                                  lr=opt.lr,
                                                  irm_coeff=opt.irm_coeff,
                                                  lip_coeff=opt.lip_coeff,
                                                  l2_coeff=opt.l2_coeff,
        ...
        discriminator = WAILDiscriminator(env, opt.d_layer_dims,
                                                  lr=opt.lr,
                                                  use_actions=opt.use_actions,
                                                  irm_coeff=opt.irm_coeff,
                                                  lip_coeff=opt.lip_coeff,
        ...
        discriminator = MEIRLDiscriminator(env, opt.d_layer_dims,
                                                  lr=opt.lr,
    ...
    if opt.train_discriminator:
        callback_on_best = RewardCheckpointCallback(discriminator=discriminator,
                                                    verbose=1, log_path=log_path,
    ...
    #eval_callback.init_callback(ppo_dict[k])
    callback_list = CallbackList([CustomCallback(id='', log_path=log_path), eval_callback])
    ...
    for i_update in tqdm(range(opt.n_irl_episodes), 'episodes'):
        ...
        policy.collect_rollouts(env, callback_list, policy.rollout_buffer,
        ...
        if opt.train_discriminator:
            for irl_epoch in range(opt.n_irl_epochs):
                ...
                for i, demo in enumerate(expert_demos):
                    update_dict = prepare_update_airl(env, opt,
                                                           demo,
                                                           policy_state_batch,
                                                           policy_action_batch,
                                                           policy)
                ...
                summary_writer.add_scalar('IRL/'+opt.discriminator_type+'_bceloss', bce_loss_all, i_update)
                if opt.irm_coeff > 0:
                    summary_writer.add_scalar('IRL/'+opt.discriminator_type+'_irmloss', grad_pen_all, i_update)
    ...
    if callback_on_best is not None:
        return plot_list, callback_on_best.plot_list_reward, callback_on_best.plot_list_value,
    else:
        return [], [], []

### EVAL functions

def test_policy(fname, model=None, rm='ERM', shift_x=0, shift_y=0, n_timesteps=2000, deterministic=True):
    if model is None:
...
if train_pwil_:
    ...
    model_pwil, plot = train_pwil(flat_demos, n_demos=3, subsampling=10, use_actions=False,
                            n_timesteps=1e3, n_targets=2, shift_x=0, shift_y=0, kwargs=kwargs,
                            fname='pwil_0', model_dir='./models', save_deterministic=False)
...
for ss in [1,2,3,5,10,20]:
    for j, dem in enumerate([demos[0], flat_demos_01, flat_demos_12, flat_demos_0]):
        for n_demos in [1,2,3]:
            ...
            env = PWILReward(env=MovePoint(2, 0, 0),
                             demos=dem,
                             n_demos=n_demos,
                             subsampling=ss,
                             use_actions=False)
            ...
            im = Image.fromarray(plot)
            im.save("pwil_plots/pwil_ss{}_demoidx{}_n_demos{}.png".format(ss,j,n_demos))
...
opt.irl_reward_model = './logs/results_2dnav_irl20220803_232801irm_1.0_train_disc/disc.th'
opt.train_discriminator = False
