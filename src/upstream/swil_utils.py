...
def get_random_projections(latent_dim: int, num_samples: int, proj_dist='normal') -> Tensor:
    ...
    if proj_dist == 'normal':
        ...
        rand_samples = torch.distributions.Cauchy(torch.tensor([0.0]),
...
def get_generalized_random_projections(projection_model, latent_dim: int, num_samples: int, proj_dist: str) -> Tensor:
    """
    Returns random samples from latent distribution's (Gaussian)
    unit sphere for projecting the encoded samples and the
    distribution samples.
...
def compute_swd(z_expert: Tensor,
                z_policy: Tensor,
                p: float,
                reg_weight: float,
                n_proj: int,
                latent_dim: int) -> Tensor:
    """
